...
## Development
### Running Tests
The test suite runs entirely on your computer. No ESP32, no Divoom device and no Bluetooth is involved. Open a terminal in the repository's root folder and run:

```
pio test -e native
```

Make sure to pass `-e native`. Without it, `pio test` builds against `esp32dev` and then waits for a board to be connected. You also need a host C++ compiler
(`g++`, or MinGW-w64 on Windows). The ESP32 cross compiler that PlatformIO installs for you does not cover this, since the tests are compiled for your machine.

Most of the tests compare the bytes generated for a MODE command against a recorded copy in `test/goldens`. These files freeze the wire protocol byte for byte,
so a red golden test means the generated bytes changed. Find out why first. If the change was intended, record the files again and review their diff as part of your commit:
